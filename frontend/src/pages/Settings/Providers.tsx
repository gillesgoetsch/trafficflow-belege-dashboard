...
export default function Providers() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["providers"] }); setCreating(false); setEditing(null); toast({ title: "Saved", variant: "success" }); },
  });
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <header className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold tracking-tight">Providers & rules</h1>
        <Button onClick={() => setCreating(true)}><Plus className="h-4 w-4 mr-1" /> Add provider</Button>
      </header>
      <Card>
        <div className="divide-y divide-border">
          {(providers ?? []).map((p) => (
            <div key={p.id} className="p-4 flex items-center gap-3">
              ...
              <Button size="sm" variant="outline" onClick={() => setOpenRulesFor(p)}>Edit rules</Button>
              <Button size="icon" variant="ghost" onClick={() => setEditing(p)}><Edit3 className="h-4 w-4" /></Button>
              <Button size="icon" variant="ghost" onClick={() => confirm("Delete provider?") && del.mutate(p.id)}><Trash2 className="h-4 w-4" /></Button>
            </div>
          ))}
        </div>
      </Card>
...
function ProviderDialog({ open, prov, onClose, onSave }: { open: boolean; prov: Provider | null; onClose: () => void; onSave: (body: any) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader><DialogTitle>{prov ? "Edit provider" : "New provider"}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div><Label>Slug</Label><Input value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-"))} /></div>
          <div><Label>Display name</Label><Input value={display_name} onChange={(e) => setDisplay(e.target.value)} /></div>
          <div className="grid grid-cols-2 gap-2">
            <div><Label>Category</Label><Input value={category} onChange={(e) => setCategory(e.target.value)} /></div>
            <div><Label>Default currency</Label><Input value={default_currency} onChange={(e) => setDc(e.target.value.toUpperCase())} /></div>
          </div>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={onClose}>Cancel</Button>
            <Button onClick={() => onSave({ slug, display_name, category, default_currency })}>Save</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
...
function RulesDialog({ provider, onClose }: { provider: Provider; onClose: () => void }) {
  ...
  const add = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["provider-rules", provider.id] }); setMatchValue(""); toast({ title: "Rule added" }); },
  });
  const del = useMutation({
  ...
  return (
    <Dialog open={true} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader><DialogTitle>Rules · {provider.display_name}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div className="flex items-end gap-2">
            ...
            <div className="flex-1"><Label>Value</Label><Input value={matchValue} onChange={(e) => setMatchValue(e.target.value)} placeholder="example.com or some keyword" /></div>
            <Button onClick={() => add.mutate()} disabled={!matchValue}>Add</Button>
          </div>
          <div className="divide-y divide-border border border-border rounded-md max-h-72 overflow-auto">
            {(rules ?? []).map((r) => (
              <div key={r.id} className="p-2.5 flex items-center justify-between text-sm">
                <div>
                  <Badge variant="outline" className="mr-2">{r.match_type}</Badge>
            ...
            {!rules?.length && <div className="p-4 text-center text-muted-foreground">No rules yet.</div>}
          </div>
        </div>
      </DialogContent>
    </Dialog>
